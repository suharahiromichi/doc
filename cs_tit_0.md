...
#### 2.4.3.2 (定理2.14) の証明図の存在と証明可能の関係から、

```N ⊨ provT(⌜A⌝) ⇔ T ⊢ A```           ..... (定理2.14)を転記する。

```N ⊨ provT(⌜GT⌝) ⇔ T ⊢ GT```         ..... GTを代入する。Aは任意なので。

provT(x)はΣ1文であるので、(式2.13)または(式2.13')を適用できる。

```T ⊢ provT(⌜GT⌝) ⇐ T ⊢ GT```         ..... (式2.13)を左辺に適用する、無矛盾な拡大のとき

```T ⊢ provT(⌜GT⌝) ⇔ T ⊢ GT```         ..... (式2.13')を左辺に適用する、1-無矛盾な拡大のとき

#### 2.4.3.3 TがQの無矛盾な拡大のとき、GTが導けないような文（ゲーデル文）が存在する。

``T ⊢ GT ⇒ T ⊢ provT(⌜GT⌝) ⇔ T ⊢ ~GT`` が得られる。
GTが導けるとすると~GTが導けてしまうので、GTが導けないことが結論できる。


#### 2.4.3.4 TがQの1-無矛盾な拡大のとき、
~GTもGTも導けないような文（ゲーデル文）が存在する。

...
4. ``p(x,f(y))`` ではなく、``p x (f x)`` のOCaml風の表記である。
...
- ISO Prologと同様に、定数は小文字から始め、変数は大文字から始めます。ただし、いわゆるラムダ変数にはその区別はありません（binding operatorの項を参照してください）。
...
- ISO Prologと違って、述語の引数の数（アリティ）によって、定義された述語を識別することをしない。
- ISO Prologと違って、オーカーチェックをしてくれます（注）。
- リストの表記はISO Prologと同じです。ただしELPIでは Coq と同じ表記（``::``や``nil``) も使えます。
- ELPIの固有の拡張として、アンダースコアから始まる名前は変数ではなく、ワイルドカードとして扱われます。

(注) オーカーチェックとは、``f X = X`` を偽にする機能のことです。ISO Prologでは歴史的な理由でチェックしない（真になり、``f (f (f (f ...))) = X``）のがディフォルトになっています。
...
特にbinding operatorの型注釈（いわゆるラムダ変数の型宣言）は無視されるようです。
...
in (``i:``) を指定した引数に対して、ゴール（呼び出し側）と、プログラム（呼び出され側）に未束縛の変数が含まれる場合、未束縛の変数どうしの場合しか、マッチが成功しません。

例えば、AとXが未束縛の変数の場合、ゴール側の引数``f A``と、プログラムがわの引数``f X``はマッチするが、ゴール側``A``とプログラム側``f X``はマッチせす、ゴール側``f A`` とプログラム側``X``はマッチしません。
...
「λ x : τ.  G」を binding operator ``\`` を使用して ``<X> : <T> \ <G>`` と書きます。ここで ``: <T>`` は型注釈です。変数``<X>``は、小文字から初めても大文字から初めても差はありません。

binding operator の優先順位（結合度）は、``,``、``;``や``:-``、``=>`` のシグネチャより低く、次節にしめす``sigma``や``pi``より高いことに注意してください。ただじ、本資料では、誤解を避けるために、冗長に括弧``()``をつける場合があります。
...
これは当然FOHCですが、省略された全称記号を補って数学記号での表記にすると、
...
束縛変数の変数名は、大文字から始まる名前でも、小文字から始まる名前でも構いません。名前としては区別されますが、``\`` の右側の式をスコープとする、という意味では同じです。

λPrologでは、FOHCの式に限り、変数名を大文字から始めることで、piなどを省略することができるので、
...
であり、大文字から始まる名前にして、sigma を省略すると、
...
また、再帰の基底にあたる ``rv nil K :- !`` で、その外側で定義された変数 ``K`` を参照しているため、単純な書き換えでISO Prologに書き直すことができません。
...
Coq（Vernacular にというべきでしょうか) に、``Elpi`` コマンドが追加され、``*.v`` ファイルのなかに直接ELPIのプログラムを書くことができるようになります。さらにそのELPIのプログラムの中からCoqのコンテキストやゴールなどにアクセスする手段が提供されるので、結果として、``*.v``ファイルのなかにELPIでタクティクを書くことができるわけです。

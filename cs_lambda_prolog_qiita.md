...
- ISO Prologと同様に、述語や定数は小文字から始め、変数を大文字から始めます。ただし、いわゆるラムダ変数にはその区別はありません（binding operatorの項を参照してください）。
...
(注) オーカーチェックとは、``f X = X`` を偽とする機能のことです。ISO Prologでは歴史的な理由でチェックしない（真になり、``f (f (f (f ...))) = X``）のがディフォルトになっています。
